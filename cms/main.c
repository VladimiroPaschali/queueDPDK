...
print_stats(void)
{
	...
	for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
		...
		uint64_t diff_dropped = port_statistics[portid].dropped - prev_dropped[portid];
		...
		printf("\nStatistics for port %u ------------------------------"
		       ...
		       portid,
		       (unsigned long long)port_statistics[portid].tx,
		       (unsigned long long)diff_tx,
		       (unsigned long long)port_statistics[portid].rx,
		       (unsigned long long)diff_rx,
		       (unsigned long long)port_statistics[portid].dropped,
...
cms_main_loop(void)
{
	...
	// fprintf(stderr,"lcore %u: n_rx_port=%u\n", lcore_id, qconf->n_rx_port);
...
main(int argc, char **argv)
{
	...
	RTE_ETH_FOREACH_DEV(portid)
	{
		...
		ret               = rte_eth_rx_queue_setup(
            portid, 0, nb_rxd, rte_eth_dev_socket_id(portid), &rxq_conf, cms_pktmbuf_pool);
		if (ret < 0)
		...
		tx_buffer[portid] = rte_zmalloc_socket(
		    "tx_buffer", RTE_ETH_TX_BUFFER_SIZE(MAX_PKT_BURST), 0, rte_eth_dev_socket_id(portid));
		...
		ret = rte_eth_tx_buffer_set_err_callback(
		    tx_buffer[portid], rte_eth_tx_buffer_count_callback, &port_statistics[portid].dropped);
		if (ret < 0)
